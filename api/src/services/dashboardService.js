...
module.exports.getTLoanCurrentStats = async () => {
    const query = `SELECT COUNT(TLoanID) AS Requests,  MONTHNAME(ApplicationDate) as Month FROM TLoan WHERE (TLoanStatusID != "1" OR TLoanStatusID != "2" OR TLoanStatusID != "9") GROUP BY MONTHNAME(ApplicationDate) ORDER BY ApplicationDate`;
...
module.exports.getRMACurrentStats = async () => {
    const query = `SELECT COUNT(RmaID) AS Requests,  MONTHNAME(DateTime) as Month FROM Rma WHERE (RmaStatusID != "3" OR RmaStatusID != "7") GROUP BY MONTHNAME(DateTime) ORDER BY DateTime;`;
    // const query = `select (select count(TLoanNumber) from TLoan) as TLoanRequests,
    // (select count(RmaID) from Rma) as RMARequest`;
    return knex.raw(query);
};
...
module.exports.getTLoanCompanies = async () => {
    const query = `
    SELECT 
    COUNT(T.TLoanID) AS Requests,
    COALESCE(C.CompanyName, "Others") AS Company
    FROM TLoan T
