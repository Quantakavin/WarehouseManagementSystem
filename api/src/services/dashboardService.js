...
module.exports.getTLoanCurrentStats = async () => {
    const query = `SELECT COUNT(TLoanNumber) AS RequestMade,  EXTRACT(MONTH FROM TLoan.ApplicationDate) as MonthNumber FROM TLoan WHERE TLoanStatusID = "3"  GROUP BY Month`;
...
module.exports.getRMACurrentStats = async () => {
    const query = `SELECT COUNT(RmaNo),  EXTRACT(MONTH FROM DateTime ) as MonthNumber FROM Rma WHERE RmaStatusID = "2" GROUP BY Month`;
    // const query = `select (select count(TLoanNumber) from TLoan) as TLoanRequests,
    // (select count(RmaNo) from Rma) as RMARequest`;
    return knex.raw(query);
};
