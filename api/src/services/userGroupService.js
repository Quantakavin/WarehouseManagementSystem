    ...
    return knex.transaction((trx) => {
        ...
        .then((ids) => {
            if (features.length > 0) {
                features.forEach((feature) => feature.UserGroupID = ids[0]);
...
module.exports.update = async (userGroupID, name, description, features) => {
    ...
    */
    return knex.transaction((trx) => {
        knex('UserGroup').where('UserGroupID', userGroupID).update({
            UserGroupName: name,
            UsergroupDesc: description
        })
        .transacting(trx)
        .then(() => {
            return knex('UserGroupFeature').where('UserGroupID', userGroupID).del().transacting(trx)
        })
        .then(() => {
            if (features.length > 0) {
                features.forEach((feature) => feature.UserGroupID = userGroupID);
                return knex('UserGroupFeature').insert(features).transacting(trx);
            }
        })
        .then(trx.commit)
...
module.exports.delete = async (userGroupID) => {
    return knex.transaction((trx) => {
        knex('UserGroupFeature').where('UserGroupID', userGroupID).del()
        .transacting(trx)
        .then(() => {
