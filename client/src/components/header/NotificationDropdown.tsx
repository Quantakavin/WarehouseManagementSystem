import { Box, Button, CircularProgress, Divider, Menu, MenuItem, Paper, Typography } from "@mui/material";
...
import { selectNotificationCount, setNotificationCount } from "../../app/reducers/NotificationSlice";
import axios from "axios";
import config from "../../config/config";
import { NotificationType } from "../../utils/CommonTypes";
import Notification from "../notifications/Notification";
import { useMutation, useQuery, useQueryClient } from "react-query";
import { GetNotifications, MarkNotificationsAsRead } from "../../api/NotificationDB";
...
const NotificationDropdown: React.FC<NotificationDropdown> = ({ menuId, anchorEl, isMenuOpen, handleMenuClose }) => {
  ...
  const NotificationsQuery = useQuery([`notifications`, userid.toString()], () => GetNotifications(userid.toString()), {
    onSuccess: (data) => {
      dispatch(setNotificationCount({ notificationCount: data.data.filter((notification) => notification.Read === 0).length }))
    }
  });

  const mutation = useMutation(() => MarkNotificationsAsRead(userid.toString()), {
    onSuccess: () => {
  ...
  return (
    ...
    >
      <Box sx={{ display: "flex", flexDirection: "row", paddingLeft: "15px", paddingRight: "10px", alignItems: "center", mb: "5px" }}>
        <Typography sx={{ fontWeight: 600 }}>Notifications {notificationcount > 0 && "(" + notificationcount + ")"}</Typography>
        {notificationcount > 0 &&
          <Button onClick={() => mutation.mutate()} sx={{ textTransform: "none", ml: "auto", fontSize: "12px" }}>Mark all as read</Button>}
      </Box>
      <Divider />
      {
        NotificationsQuery.status === "success" ?
          <div>
            {NotificationsQuery.data.data.filter((notification) => notification.Read === 0).length !== 0 ?
              <div>
                {NotificationsQuery.data.data.filter((notification) => notification.Read === 0).slice(0, 5).map((notification) => {
                  return <MenuItem key={notification.NotificationID}><Notification maxWidth="220px" notification={notification} /></MenuItem>
                })}
              </div> 
              : 
              <div className="flexcontainer"><Typography sx={{ mt: "10px", mb: "10px", color: "#0a2540" }}>No unread notifications</Typography></div>
            }
            {NotificationsQuery.data.data.length > 0 && 
            <div>
            <Divider sx={{width:'100%'}}/>
            <div className="flexcontainer">
            <Button onClick={() =>  navigate("/notifications")}sx={{ textTransform: "none", fontSize: "15px", color: "#0a2540", mb:"-5px" }}>View all notifications</Button>
            </div>
            </div>
            }
          </div>
