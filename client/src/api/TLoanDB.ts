...
export const GetPending = async ({pageParam = 0}) => {
    const response = await axios.get(`${config.baseURL}/tloan/pending?limit=5&page=${pageParam * 5}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
...
export const GetDraft = async ({pageParam = 0}) => {
    const response = await axios.get(`${config.baseURL}/tloan/drafts?limit=5&page=${pageParam * 5}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
...
export const GetHistory = async ({pageParam = 0}) => {
    const response = await axios.get(`${config.baseURL}/tloan/history?limit=5&page=${pageParam * 5}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
