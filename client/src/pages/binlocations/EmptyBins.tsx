...
const EmptyBins: React.FC = () => {
    ...
    return (
        <Box sx={{ pl: 3, pr: 3, pt: 1, height: "100%", width: "100%" }}>
            ...
            <Box style={{ backgroundColor: "white", border: "solid 1px #d3d3d3", borderRadius: "5px", display: "block", height: "650px" }}>
                <button
                    style={{ alignSelf: "flex-start", marginTop: "10px", marginLeft: "10px" }}
                ...
                {EmptyBinsQuery.status === "success" && (
                    <div style={{maxHeight: "580px", overflow: "auto",}}>
                        {EmptyBinsQuery.data.data.length !== 0 ? (
                            <div className="flexcontainer" style={{ flexDirection: "column", paddingTop: "40px", paddingBottom: '40px' }}>
                                {EmptyBinsQuery.data.data.map((bin) => {
                                    return (
                                        <Box
                                            ...
                                            key={bin.BinID}
                                        >
                                            <Typography sx={{ color: "#0A2540", fontWeight: 600 }}>{bin.BinTag2}</Typography>
                                        </Box>
                                    );
                                })}
                            </div>
