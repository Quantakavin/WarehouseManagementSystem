              ...
              {NotificationsQuery.data.data.length !== 0 ?
                <div>
                  {NotificationsQuery.data.data.map((notification) => {
                    return <Box sx={{ backgroundColor: "white", border: "solid 1px #d3d3d3", borderRadius: "5px", mb: "10px", '&:hover': { backgroundColor: "#ededed" } }} key={notification.NotificationID}><Notification maxWidth="100%" notification={notification} /></Box>
                  })}</div> : <div className="flexcontainer"><Typography sx={{ mt: "10px", mb: "10px", color: "#0a2540" }}>No notifications</Typography></div>}
            </div> :
            <div>
              {NotificationsQuery.data.data.filter((notification) => notification.Read === 0).length !== 0 ?
                <div>
                  {NotificationsQuery.data.data.filter((notification) => notification.Read === 0).map((notification) => {
                    return <Box sx={{ backgroundColor: "white", border: "solid 1px #d3d3d3", borderRadius: "5px", mb: "10px", '&:hover': { backgroundColor: "#ededed" } }} key={notification.NotificationID}><Notification maxWidth="100%" notification={notification} /></Box>
                  })}</div> : <div className="flexcontainer"><Typography sx={{ mt: "10px", mb: "10px", color: "#0a2540" }}>No unread notifications</Typography></div>}
            </div>
          }
        </div>
      }
